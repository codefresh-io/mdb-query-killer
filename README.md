...
| thresholdSeconds  | Time in seconds a mongo operation must be running to trigger the killer|
| checkIntervalSeconds |  Time in seconds between the checks   |
| killFilter  | Defines a JSON schema, which is matched against a mongo operation object. If there is no match, operation will not be killed |
| recordAllLongOps | By default, only the killed operations are recorded. If this flag is `true`, all the operations above the threshold value will be recorded, not only the killed ones|
| longOpsDB  |  Defines a name of the mongo database, where the long running operations are recorded to |
| longOpsCollection | Relevant only with the `recordAllLongOps` flag set. Defines a name for the mongo collection, where the long running operations are recorded to |
| killedOpsCollection | Defines a name for the mongo collection, where the long running operations are recorded to right before they are killed |
|killingEnabled|Enables/Disables killing of the operations|
